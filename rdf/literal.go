...
func init() {
	...
	if RegionLiteral, err = literal.DefaultBuilder().Build(literal.Text, Region.ToRDFType()); err != nil {
		panic(err)
	}
	...
	if InstanceLiteral, err = literal.DefaultBuilder().Build(literal.Text, Instance.ToRDFType()); err != nil {
		panic(err)
	}
}
