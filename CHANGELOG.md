...
- Add suggested params: optional but very common params that are prompted by default. Can be disabled with `--prompt-only-required` or forced with `--prompt-all-params` to leverage smart completion for all optional params
